...
**Deadline hackathon resmi:** 30 September 2026 (periode lomba 1–30 September 2026). Per 25 September 2026 tersisa 5 hari kalender. Sumber: halaman resmi AssemblyAI Voice Agent Hackathon di lablab.ai.
...
| **Pemilik sesi diambil dari token, bukan dari body** | Kalau klien boleh menyebutkan sendiri siapa dirinya, atribusi laporan tidak membuktikan apa pun — padahal klaim utama produk ini adalah bukti |
| **Peran WebSocket ditentukan token, bukan query** | Sebelumnya klien menulis `role=officer` sendiri, jadi siapa pun yang tahu ID sesi bisa mendorong audio ke percakapan orang lain |
| **Token HMAC sendiri, bukan JWT** | Yang dibutuhkan hanya satu algoritma dan tiga klaim. JWT membawa negosiasi algoritma — sumber kerentanan klasik `alg: none` — beserta satu dependensi lagi |
| **PBKDF2 dari pustaka standar, bukan bcrypt/argon2** | `crypto/pbkdf2` ada di Go 1.24+ sehingga tidak perlu dependensi baru. Argon2id lebih disukai untuk kata sandi tetapi hanya tersedia lewat `golang.org/x/crypto` |
| **`AUTH_SECRET` tanpa nilai bawaan** | Rahasia bawaan yang ikut ter-commit berarti siapa pun bisa menandatangani token petugas mana pun. Gagal start lebih baik |
| **Akun seed hanya dibuat saat tabel users kosong** | Kalau tidak, mengubah variabel lingkungan akan diam-diam menimpa kata sandi yang sudah dipakai, dan kata sandi bawaan hidup selamanya |
...
### 🟠 Jalur kritis
- [x] **Autentikasi petugas & supervisor.** Diminta pemilik repo supaya kedua peran bisa diuji terpisah. Backend, PWA, dan Flutter selesai; langkah ujinya di `TESTING.md` §2b–2c.
- [ ] Supervisor belum melihat **progres** yang sudah terjadi sebelum ia bergabung. Daftar butir kewajiban kini dimuat saat mulai memantau, tetapi status checklist-nya belum di-snapshot dari database.
...
- [x] README ditulis ulang untuk pembaca luar: demo terarah, desain gagal-dengan-aman, privasi, model kepercayaan tiga aktor, dan batasan yang disadari.
- [ ] Jalankan Flutter di perangkat fisik dengan `--dart-define=API_URL=http://<IP-LAN>:8080`. Splash, ikon, animasi memuat, dan banner error sudah dibuat tetapi belum pernah tampil di perangkat sungguhan.
...
| 2026-09-25 | Bug lanjutan dari refactor auth diperbaiki: klik "Mulai sesi" gagal dengan "ID petugas wajib diisi". Penyebabnya validasi lama di `StartSessionUseCase` (web dan Flutter) yang masih mewajibkan `officerId`, padahal klien sudah tidak mengirimnya lagi sejak pemilik sesi diambil dari token — permintaan dihentikan di klien sebelum sempat sampai backend. `officerId` dihapus dari seluruh rantai: use case, kontrak repository, implementasi, datasource, dan controller. Kolom "ID Petugas" di Flutter ikut diganti teks "Masuk sebagai <nama>", karena membiarkan petugas mengetik ID siapa pun persis lubang atribusi yang baru saja ditutup. Ditambah **logging request HTTP** di backend (`withLogging`): metode, path, status, dan durasi, dengan 4xx sebagai WARN dan 5xx sebagai ERROR, serta health check yang sehat dilewati agar log tidak tenggelam. Pembungkus `ResponseWriter`-nya meneruskan `Hijack` — tanpa itu upgrade WebSocket mati total — dan hal itu dikunci test tersendiri. 3 test baru. Verifikasi: `go test ./...` ✅ · `go vet ./...` ✅ · `gofmt` bersih ✅ · `pnpm lint` ✅ · `pnpm build` ✅ · `flutter analyze` ✅. |
| 2026-09-25 | Jarak form login diperbaiki: `.start` memberi gap 8px seragam sehingga label, input, dan tombol sama-sama berdempetan — kata sandi menempel ke tombol Masuk. Ditambah `.field` (label menempel input, 7px) dan `.start--login` (antar kelompok 18px, tombol diberi jarak tambahan). Diperiksa visual dari CSS hasil build. Selain itu `saksi_backend/.env` lokal ternyata belum memuat satu pun kunci auth — hanya `.env.example` yang diperbarui — sehingga backend menolak start dan tidak ada akun yang pernah dibuat. `AUTH_SECRET` acak dan kata sandi seed lokal sudah diisi; nilainya tidak dicatat di sini dan `.env` tetap diabaikan Git. |
| 2026-09-25 | Autentikasi petugas dan supervisor selesai di tiga project, atas permintaan pemilik repo supaya kedua peran bisa diuji terpisah. **Backend:** tabel `users` (migrasi `003`), PBKDF2-HMAC-SHA256 600k iterasi dari pustaka standar Go 1.24+ (tanpa dependensi baru), token bertanda tangan HMAC `<payload>.<sig>` alih-alih JWT, middleware `requireAuth(verifier, roles...)`, dan `SocketAuthorizer` untuk WebSocket. Dua lubang atribusi ditutup: pemilik sesi kini diambil dari token alih-alih body, dan peran WebSocket ditentukan token alih-alih query `role` — sebelumnya siapa pun yang tahu ID sesi bisa mengaku petugas lalu mendorong audio ke percakapan orang lain. Ditambah `GET /api/sessions` khusus supervisor supaya ID sesi tidak perlu disalin dari psql. **PWA:** login di kedua halaman, token di header REST dan query `/ws`, pemulihan sesi login lewat `GET /api/auth/me`, logout, dan daftar sesi untuk supervisor. Mode demo terarah sengaja tetap terbuka tanpa login karena juri harus bisa mencobanya tanpa kredensial. **Flutter:** halaman login, interceptor Dio, token di query WebSocket, logout di AppBar; token hanya di memori karena tidak ada plugin penyimpanan terenkripsi di dependensi — petugas masuk ulang tiap aplikasi dibuka. `render.yaml`, `.env.example`, `make auth-secret`, README, dan `TESTING.md` §2b–2c ikut diperbarui. 9 test baru. Verifikasi: `go test ./...` ✅ · `go vet ./...` ✅ · `gofmt` bersih ✅ · `pnpm lint` ✅ · `pnpm build` ✅ · `make deploy-build` ✅ · `flutter analyze` ✅. **Belum diuji terhadap backend sungguhan** — alur login end-to-end menunggu pengujian pemilik repo. |
| 2026-09-25 | README ditulis ulang untuk juri, bukan untuk kontributor. Tiga kekuatan yang sebelumnya tidak muncul sama sekali kini jadi bagian utama: mode demo terarah 9 detik yang berjalan tanpa backend maupun API (sebelumnya hanya tercatat di `TESTING.md`), tabel **cara sistem ini gagal dengan aman** yang mengumpulkan evidence gate, guardrail deterministik, kalibrasi pembicara, pembicara ketiga, quality gate audio, dan penilaian ulang setelah revisi diarization ke satu tempat dengan prinsipnya dinyatakan terang-terangan, serta fakta bahwa **audio mentah tidak pernah disimpan** (sebelumnya terkubur di `SCHEMA.md`). Ditambah **model kepercayaan** tiga aktor — petugas dan supervisor login, nasabah tidak pernah memakai aplikasi — beserta pengakuan eksplisit bahwa autentikasi di luar scope dan `officer_id` masih kolom teks bebas sehingga atribusi laporan belum terbukti. Bagian **batasan yang disadari** menyebut jalur live yang belum terbukti, overlap satu mikrofon, ambang quality gate yang belum dikalibrasi, dan ketertinggalan Flutter. Semua tautan file diverifikasi. Efek samping dari banner error baru ikut diperbaiki: kegagalan memuat daftar kewajiban saat mount tidak lagi memakai `sessionFailed` — itu bukan kegagalan sesi, dan kotak merah saat halaman dibuka membuat pengunjung mengira aplikasinya rusak padahal demo terarah tetap berjalan penuh. Sekarang tampil sebagai peringatan yang menyebutkan demo tetap bisa diputar. Verifikasi: `pnpm lint` ✅ · `pnpm build` ✅ · `make deploy-build` ✅. |
| 2026-09-25 | Bug "tombol Mulai sesi tidak jalan" di Flutter ditemukan penyebabnya: **bukan satu bug, tetapi dua yang bergabung**. Pertama, `_buildIdle` tidak pernah menerima maupun menampilkan `state.error`, jadi kegagalan `start()` tersimpan diam-diam di state dan layar tidak berubah sama sekali. Kedua, tidak ada penanda proses apa pun, padahal membuka sesi butuh HTTP (timeout Dio 8 detik) lalu WebSocket lalu izin mikrofon — jadi walau berhasil, tombolnya tetap terasa mati beberapa detik. Pemicu sebenarnya hampir pasti alamat gateway: bawaan `10.0.2.2` hanya benar untuk emulator Android dan tidak berarti apa-apa di iPhone fisik. Perbaikan: `SessionState.starting`, tombol dinonaktifkan + animasi selama proses, banner error di layar idle, dan pesan `DioException` ditulis ulang agar menyebut alamat yang dicoba beserta cara memperbaikinya. **Cacat yang sama ternyata ada di web** dan ikut diperbaiki: `handleStart` async tanpa penanda proses, dan `sessionFailed` tidak pernah dirender di layar mulai. | 
| 2026-09-25 | Splash screen dan animasi memuat dibuat sesuai palet Bisik. Sebelumnya splash Android putih bawaan scaffold dan `NormalTheme` memakai `?android:colorBackground`, jadi ada dua kedipan terang sebelum UI gelap muncul; iOS sama, latar storyboard putih. Sekarang Android memakai `@color/bisik_bg` + logo di `launch_background.xml` (dan `drawable-v21`), iOS memakai latar `#0B1211` di storyboard dengan `LaunchImage` 96/192/288 px dari logo, dan web mendapat splash SVG inline di `index.html` yang digantikan React saat mount. Manifest PWA serta `<meta name="theme-color">` masih memakai `#0d1117` dari palet GitHub lama dan ikut disamakan ke `#0b1211`. Animasi memuat `BisikWave`: lima batang bergelombang yang bentuknya diambil dari ikon — kebetulan lima juga jumlah butir kewajiban, jadi terbaca sebagai "sedang menyimak", bukan spinner generik. Dibuat kembar di Flutter (`lib/core/theme/bisik_wave.dart`) dan web (`BisikWave.tsx` + `.wave` di `index.css`, memakai `currentColor` agar benar di atas tombol mint maupun latar gelap, dan berhenti pada `prefers-reduced-motion`). Verifikasi: `flutter analyze` ✅ · `pnpm lint` ✅ · `pnpm build` ✅ · `make deploy-build` ✅ · `go test ./...` ✅ · `go vet ./...` ✅ · splash web dan state memuat diperiksa visual dari hasil build. **Belum dijalankan di perangkat.** |
...
| 2026-09-25 | Deadline hackathon diverifikasi dari halaman resmi lablab.ai: periode 1–30 September 2026, deadline 30 September 2026; tersisa 5 hari kalender pada tanggal verifikasi. |
